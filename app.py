...
def generate_pdf_report(df, charts):
    ...
    pdf.cell(0, 10, "Telecom Customer Feedback Report", ln=True, align="C")
    pdf.ln(10)

    def add_chart(title, description, image_path):
    ...
    for chart in charts:
        add_chart(chart["title"], chart["desc"], chart["file"])
...
if uploaded_file:
    ...
    if "Location" in df.columns:
        ...
        region_fig.write_image("region.png", width=1000, height=600, scale=2)
    ...
    if "CustomerFeedback" in df.columns:
        ...
        fig_gauge.write_image("gauge.png", width=1000, height=600, scale=2)
        ...
        ax.axis("off")
        wc_fig.savefig("wordcloud.png", dpi=300, bbox_inches="tight")   # <-- FIX: save wordcloud
        st.pyplot(wc_fig)
